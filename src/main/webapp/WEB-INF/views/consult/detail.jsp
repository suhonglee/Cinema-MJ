			...
			<div class="dpbd-inform">
				<table class="dpbd-table" width="100%">
					...
					<div class="dpbd-inform-right">
						<span class="dpbd-inform-button"><a href='${CONTEXT }/consult/'>목 록</a></span>
				...
				<table class="dpbd-header-table" width="100%">
					<tbody>
						<tr>
							<td class="table-title-td"><strong>작 성 자 :</strong></td>
							<td>${consult.writer }</td>
							<td class="table-title-td"><strong>연 락 처 :</strong></td>
							<td>${consult.tel }</td>
							<td class="table-title-td"><strong>이 메 일 :</strong></td>
							<td>${consult.email }</td>
							<td class="table-title-td"><strong>작 성 일 :</strong></td>
							<td>${consult.time }</td>
						</tr>
					</tbody>
				</table>
	...
	<script>
		...
		function deleteConfirm()
		{
			$.ajax({
				...
				var isAdmin = data;
				if(! isAdmin && ${consult.secret == false})
				{
				...
				if(confirm('정말 삭제하시겠습니까?'))
				{
					$.ajax({
						type:"POST"
						,url: "${CONTEXT}/consult/${no}"
						,data: {"_method":"DELETE"}
					}).done(function(data){
						if(data==true)
						{
							location.replace('${CONTEXT}/consult/');
						}
						else
						{
							alert('글 삭제에 실패하였습니다');
		...
		function deleteReply(id, no)
		{
			$.ajax({
				...
				if(! isAdmin && ${consult.secret == true})
				{
					password = prompt("댓글 작성 시 사용한 비밀번호를 입력하세요");
					if(password==null) return ;
				}
				else if(! isAdmin && ${consult.secret != true})
				{
					alert('삭제 권한이 없습니다');
					return ;
				}
