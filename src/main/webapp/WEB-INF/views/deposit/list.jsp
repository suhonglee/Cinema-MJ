	...
	<div class="contents-area">
		...
		-->
			...
			<div class="bbslist-item-wrapper " >
				<c:forEach items="${dataList }" var="deposit">
					<ul class="bbslist-item" >
						<li><span id="no">${deposit.no }</span></li>
						<li><span><a href="${CONTEXT }/deposit/${deposit.no}">${deposit.title }</a></span></li>
						<li><span>${deposit.writer }</span></li>
						<li><span>${deposit.time }</span></li>
					</ul>
				</c:forEach>
			</div>
			<div class="bbs-page">
				<c:if test="${pageCount>1 }">
					<a href="${CONTEXT }/deposit/?page=${firstPage-1 }">[이전]</a>
				</c:if>
				<c:forEach begin="${firstPage }" end="${firstPage+pageListSize-1 }" var="cnt">
					<c:if test="${cnt<=pageSize }">
						<c:if test="${currentPage != cnt}">
							<a href="${CONTEXT }/deposit/?page=${cnt}">[ ${cnt} ]</a>
						</c:if>
						<c:if test="${currentPage == cnt}">
							<b>[ ${cnt } ]</b>
						</c:if>
				...
				<c:if test="${firstPage+pageListSize-1 < pageSize}">
					<a href="${CONTEXT }/deposit/?page=${firstPage+pageListSize}">[다음]</a>
				</c:if>
			</div>
			
			<div class="bbslist-inform-right">
					<p>
						<span class="bbslist-inform-button"><a href="${CONTEXT }/deposit/write">WRITE</a></span>
